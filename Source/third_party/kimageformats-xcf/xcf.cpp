...
private:
    ...
    bool malformed_tile_data = false;
    ...
    bool loadTileZlib(QDataStream &xcf_io, uchar *tile, int size, int data_length,
                      qint32 bpp, bool allow_trailing_input, qint64 *bytesParsed);
...
bool XCFImageFormat::loadLevel(QDataStream &xcf_io, Layer &layer, qint32 bpp, const GimpPrecision precision)
{
    ...
    for (uint j = 0; j < layer.nrows; j++) {
        for (uint i = 0; i < layer.ncols; i++) {
            ...
            const bool finalTile = offset2 == 0;
            if (finalTile) {
            ...
            case COMPRESS_NONE: {
                const int size = layer.image_tiles[j][i].width() * layer.image_tiles[j][i].height();
                int step = sizeof(QRgb);
                if (bpp == 6 || bpp == 8) {
                    step = sizeof(QRgb) * 2;
                } else if (bpp == 12 || bpp == 16) {
                    step = sizeof(QRgb) * 4;
                } else if (bpp < 1 || bpp > 4) {
                    malformed_tile_data = true;
                    return false;
                }
                const int dataSize = size * bpp;
                const int unpackedSize = size * step;
                if (offset2 - offset < dataSize ||
                    (needConvert && unpackedSize > buffer.size()) ||
                    (!needConvert && size_t(unpackedSize) > sizeof(layer.tile))) {
                    qCDebug(XCFPLUGIN) << "Uncompressed tile exceeds its bounded input or output";
                    malformed_tile_data = true;
                    return false;
                }
                QByteArray raw(dataSize, Qt::Uninitialized);
                if (xcf_io.readRawData(raw.data(), dataSize) != dataSize) {
                    qCDebug(XCFPLUGIN) << "Short uncompressed tile read";
                    malformed_tile_data = true;
                    return false;
                }
                auto *destination = needConvert ? buffer.data() : layer.tile;
                memset(destination, 0, size_t(unpackedSize));
                for (int pixel = 0; pixel < size; ++pixel) {
                    memcpy(destination + pixel * step, raw.constData() + pixel * bpp, size_t(bpp));
                }
                bytesParsed = unpackedSize;
                break;
            }
            case COMPRESS_RLE: {
            ...
            case COMPRESS_ZLIB: {
                const int size = layer.image_tiles[j][i].width() * layer.image_tiles[j][i].height();
                const uint step = bpp <= 4 ? sizeof(QRgb) : (bpp <= 8 ? sizeof(QRgb) * 2 : sizeof(QRgb) * 4);
                const quint64 unpackedSize = quint64(size) * step;
                if ((needConvert && unpackedSize > quint64(buffer.size())) ||
                    (!needConvert && unpackedSize > sizeof(layer.tile))) {
                    qCDebug(XCFPLUGIN) << "Zlib tile data exceeds its bounded output buffer";
                    malformed_tile_data = true;
                    return false;
                }
                qint64 dataLength = offset2 - offset;
                if (finalTile) {
                    const qint64 available = xcf_io.device()->size() - offset;
                    dataLength = qMin(dataLength, available);
                }
                if (dataLength <= 0 || dataLength > std::numeric_limits<int>::max() ||
                    !loadTileZlib(xcf_io, needConvert ? buffer.data() : layer.tile, size,
                                  int(dataLength), bpp, finalTile, &bytesParsed)) {
                    qCDebug(XCFPLUGIN) << "Failed to read zlib tile";
                    malformed_tile_data = true;
                    return false;
                }
                break;
 ...
 * the RLE data.
 */
bool XCFImageFormat::loadTileZlib(QDataStream &xcf_io, uchar *tile, int image_size,
                                  int data_length, qint32 bpp, bool allow_trailing_input,
                                  qint64 *bytesParsed)
{
    int step = sizeof(QRgb);
    switch (bpp) {
    ...
    if (image_size <= 0 || data_length <= 0 ||
        data_length > int(TILE_WIDTH * TILE_HEIGHT * step * 1.5)) {
        return false;
    }

    QByteArray compressed(data_length, Qt::Uninitialized);
    if (xcf_io.readRawData(compressed.data(), data_length) != data_length) {
        return false;
    }

    const qsizetype rawSize = qsizetype(image_size) * bpp;
    QByteArray raw(rawSize, Qt::Uninitialized);
    tinfl_decompressor decompressor{};
    tinfl_init(&decompressor);
    size_t inputBytes = size_t(compressed.size());
    size_t outputBytes = size_t(raw.size());
    const auto status = tinfl_decompress(
        &decompressor, reinterpret_cast<const mz_uint8 *>(compressed.constData()), &inputBytes,
        reinterpret_cast<mz_uint8 *>(raw.data()), reinterpret_cast<mz_uint8 *>(raw.data()),
        &outputBytes, TINFL_FLAG_PARSE_ZLIB_HEADER | TINFL_FLAG_USING_NON_WRAPPING_OUTPUT_BUF);
    if (status != TINFL_STATUS_DONE || outputBytes != size_t(raw.size()) ||
        (!allow_trailing_input && inputBytes != size_t(compressed.size()))) {
        return false;
    }

    memset(tile, 0, size_t(image_size) * size_t(step));
    for (int pixel = 0; pixel < image_size; ++pixel) {
        memcpy(tile + pixel * step, raw.constData() + pixel * bpp, size_t(bpp));
    }
    *bytesParsed = qint64(image_size) * step;
    return true;
}
...
bool XCFHandler::read(QImage *image)
{
    ...
    m_resourceLimitExceeded = xcfif.resourceLimitExceeded();
    m_unsupportedCompression = xcfif.unsupportedCompression();
    m_malformedTileData = xcfif.malformedTileData();
    m_imageSize = image->size();
    return ok;
}

bool XCFHandler::resourceLimitExceeded() const noexcept
{
    return m_resourceLimitExceeded;
}

bool XCFHandler::unsupportedCompression() const noexcept
{
    return m_unsupportedCompression;
}

bool XCFHandler::malformedTileData() const noexcept
{
    return m_malformedTileData;
}
